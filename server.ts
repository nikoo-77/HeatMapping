...
const SUPABASE_SECRET_KEY = process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_PUBLISHABLE_KEY || process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || '';
...
async function loadEmployees(): Promise<Employee[]> {
  ...
  if (!supabase) {
    throw new Error('Supabase client is not initialized. Please ensure SUPABASE_URL and SUPABASE_SECRET_KEY are set in the environment variables.');
  }
...
// ── Lazy loader middleware for database cache ──────────────────────────────────
...
app.patch('/api/employees/:id/profile', async (req, res) => {
  ...
  if (hasDbUpdate) {
    if (!supabase) {
      return res.status(500).json({ message: 'Supabase client is not initialized.' });
    }
...
if (!process.env.VERCEL) {
  loadEmployees()
    ...
    .catch((err) => {
      console.error('Failed to load employees from Supabase at startup:', err);
      process.exit(1);
    });
}
